 ...
 *
 *
 * @example jQuery('#MySplitter').splitter();
 * @desc Create a vertical splitter with default settings 
 *
 * @example jQuery('#MySplitter').splitter({type: 'h', accessKey: 'M'});
...
jQuery.fn.splitter = function(args){
	...
	return this.each(function() {
		//if ( jQuery(this).is(".splitter") ) return;	// already a splitter
		if ( jQuery(this).attr("data-splitter-initialized") ) return
        ...
        var splitter = jQuery(this).css({position: "relative"}).addClass(opts.splitterClass).attr("data-splitter-initialized",true)
        var panes = jQuery(">*", splitter[0]).addClass(opts.paneClass).css({
            position: "absolute",           // positioned inside splitter container
            "z-index": "1",                 // splitbar is positioned above
            "-moz-outline-style": "none"    // don't show dotted outline
        });
        var A = jQuery(panes[0]), B = jQuery(panes[1]);   // A = left/top, B = right/bottom
        ...
        var bar = jQuery('<div></div>')
            ...
            .bind("mouseover"+opts.eventNamespace, function(){
                jQuery(this).addClass(opts.barHoverClass);
            })
            .bind("mouseout"+opts.eventNamespace, function(){
                jQuery(this).removeClass(opts.barHoverClass);
            });
        // Use our cursor unless the style specifies a non-default cursor
        if ( /^(auto|default|)jQuery/.test(bar.css("cursor")) )
		...
		jQuery.each([A,B], function(){
			...
			this._init = opts["size"+this._pane]===true ?
                parseInt(jQuery.css(this[0],opts.split),10) : opts["size"+this._pane];
		});
		...
		if ( opts.cookie ) {
			...
			initPos = parseInt(jQuery.cookie(opts.cookie),10);
			jQuery(window).bind("unload"+opts.eventNamespace, function(){
		...
		if ( opts.resizeTo ) {
			...
			jQuery(window).bind("resize"+opts.eventNamespace, function(){
				var top = splitter.offset().top;
				var eh = jQuery(opts.resizeTo).height();
		...
		if ( opts.dock ) {
			...
			if ( opts.dockKey )
				jQuery('<a title="'+opts.splitbarClass+' toggle dock" href="javascript:void(0)"></a>')
		...
		splitter
			.bind("destroy"+opts.eventNamespace, function(){
				jQuery([window, document]).unbind(opts.eventNamespace);
				bar.unbind().remove();
